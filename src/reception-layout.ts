...
export function getDefaultReceptionCardSpan(totalLanes: number): number {
    if (totalLanes >= 12) {
        return 12;
    }
    if (totalLanes >= 8) {
...
export function createDefaultReceptionLayout(rooms: RoomConfig[]): ReceptionLayoutConfig {
    const layoutRooms: ReceptionRoomLayout[] = [];
    let currentX = 0;
    let currentY = 0;
    let rowHeight = 1;
        ...
        rooms: fallbackLayout.rooms.map((fallbackRoom) => {
            ...
            return {
                ...
                tileColumns: clampInteger(
                    Number(rawRoom.tileColumns ?? fallbackRoom.tileColumns),
                    1,
                    Math.min(RECEPTION_LAYOUT_MAX_TILE_COLUMNS, Math.max(sourceRoom.lanes, 1))
                )
            };
        })
    };

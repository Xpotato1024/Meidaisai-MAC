...
import {
    ...
    saveAdminSettings,
    updateAccessMember,
    updateLaneCustomName,
    updateLanePauseReason,
    updateLaneStatus,
 ...
 * 一覧の描画 (検索フィルタ対応)
 */
function renderEventList(context: AppContext, dataList: RegistryItem[]): void {
 ...
 * イベント台帳を取得して一覧表示
 */
async function fetchAndRenderEventList(context: AppContext): Promise<void> {
    await fetchRegistryItems(context);
    renderEventList(context, context.state.registryCache);
}

function validateAppId(currentAppId: string, id: string): string | null {
    if (!id) {
        return "IDを入力してください";
    }
    if (id === currentAppId) {
...
async function exportFullBackup(context: AppContext): Promise<void> {
    ...
    try {
        ...
        const lanesData = lanesSnap.docs.map((docSnap: any) => ({
            _id: docSnap.id,
            ...docSnap.data()
        }));
        ...
        const roomStateData = roomStateSnap.docs.map((docSnap: any) => ({
            _id: docSnap.id,
            ...docSnap.data()
        }));
...
async function importBackup(context: AppContext, file: File): Promise<void> {
    const { db, currentAppId, dom, paths, state } = context;
    ...
    reader.onload = async (loadEvent) => {
        try {
            ...
            if (isFullBackup) {
                ...
                json.roomState.forEach((roomStateData: any) => {
                ...
                batch.set(
                    doc(db, paths.registryCollectionPath, currentAppId),
                    {
                        appId: currentAppId,
...
async function copyAndSwitchAppId(context: AppContext, newId: string): Promise<void> {
    const { db, currentAppId, dom, paths, state } = context;
    ...
    try {
        ...
        // 2. Lanes書き込み
        sourceLanesSnap.forEach((docSnap: any) => {
        ...
        // 3. RoomState書き込み
        sourceRoomStateSnap.forEach((docSnap: any) => {
            const data = docSnap.data();
...
export function setupEventListeners(context: AppContext): void {
    ...
    dom.dbSearchInput.addEventListener("input", (event) => {
        ...
        const filtered = state.registryCache.filter((item) =>
    ...
    dom.dbEventList.addEventListener("click", (event) => {
        ...
        const button = target.closest('button[data-action="switch-app-id"]') as HTMLButtonElement | null;
        if (!button) {
            return;
        }
    ...
    dom.tabAdmin.addEventListener("click", (event) => {
        ...
        if (actionButton === "save-access-member" && button) {
            ...
            const roleSelect = memberCard?.querySelector<HTMLSelectElement>(`select[data-role-input][data-uid="${uid}"]`);

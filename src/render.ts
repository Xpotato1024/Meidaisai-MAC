...
import {
    getReceptionDisplayCardHeightPx,
    getReceptionRoomLayout,
...
function getReceptionRoomLaneVisuals(
    lanes: Array<{ docId: string; data: LaneData }>,
    totalLanes: number
): Array<{
    laneNumber: number;
    tileClass: string;
    ...
    const visualsByLane = new Map<number, { laneNumber: number; tileClass: string; icon: string; label: string }>();
    ...
    const visuals: Array<{ laneNumber: number; tileClass: string; icon: string; label: string }> = [];
    for (let laneNumber = 1; laneNumber <= totalLanes; laneNumber += 1) {
        visuals.push(
            visualsByLane.get(laneNumber) || {
                laneNumber,
...
function renderReceptionList(context: AppContext): void {
    ...
    const packedLayout = packReceptionRoomLayout(
        visibleRooms,
...
export async function openReceptionRoomModal(context: AppContext, roomId: string): Promise<void> {
    ...
    const renderModalContent = () => {
        ...
        dom.receptionModalContent.querySelectorAll<HTMLInputElement>(".reception-opt-chk").forEach((checkbox) => {
            checkbox.onchange = () => {
                selectedOptions = Array.from(dom.receptionModalContent.querySelectorAll<HTMLInputElement>(".reception-opt-chk:checked"))
                    .map((item) => item.value);
            };
        });

        const notesInput = dom.receptionModalContent.querySelector<HTMLInputElement>("#reception-modal-notes");
        if (notesInput) {
            notesInput.oninput = () => {
...
export function renderStaffLaneDashboard(context: AppContext, selectedRoomId: string): void {
    ...
    roomLanes.forEach((lane) => {
        ...
        const pauseReason = laneData.pauseReasonId
            ...
            : null;
        const laneStatusDisplay = escapeHtml(
            laneData.status === "paused" && pauseReason
                ? pauseReason.name
        ...
        const pauseButton = pauseStatus ? `
            <button data-action="set-lane-status" data-docid="${docId}" data-status="${pauseStatus.id}" 
                    class="status-action-button status-action-pause-row ${
                        laneData.status === pauseStatus.id

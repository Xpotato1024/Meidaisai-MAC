...
このアプリは Google ログイン後に管理者承認を受けて利用する運用です。
...
- `/artifacts/{appId}/private/accessRequests/{uid}`
  初回ログイン時の承認待ち申請
- `/artifacts/{appId}/private/accessMembers/{uid}`
...
3. Firestore Console で `/artifacts/{appId}/private/accessMembers/{uid}` を作成する
4. 次の値を入れる
...
以後のメンバー承認はアプリ内の「メンバー権限管理」から行えます。

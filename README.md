...
無料枠での運用を意識し、2026-03-09 時点で次の最適化を入れています。

- `roomState` は待機組数に加えて部屋サマリも保持し、受付画面はこのサマリをリアルタイム購読します
- 受付画面は常時 `lanes` 全件を購読せず、案内操作時だけ対象部屋のレーン一覧を取得します
- `staff` は担当部屋だけを購読します
- 待機組数更新は transaction で加減算し、同時更新の取りこぼしを防ぎます
- レーン更新は Firestore 上の最新値を読んでから `revision` を進めるため、遅延した stale write が上書きしにくい構成です
- Firestore local cache を有効化し、再接続時の read を抑えます

この構成でも `admin` は全レーンを購読します。無料枠をさらに詰める場合は、管理画面の購読も分割するか、Functions 導入を検討してください。
...
最初の 1 人だけは Firebase Console から手動で global admin を作成します。
この作業は `app_id` ごとではなく、原則 1 回だけです。

1. Firebase Authentication で Google プロバイダを有効化する
2. 管理者にしたい Google アカウントで一度ログインする
3. Firestore Console で `/sys_globalAccessMembers/{uid}` を作成する
4. 次の値を入れる
...
1. 初期 `admin` で任意の `app_id` にログインする
2. Google スプレッドシートから CSV を出力して「名簿インポート」を実行する
3. 一般メンバーは自動承認される
4. role と担当部屋はアプリ内の「メンバー権限管理」で付与する
5. 名簿外の特別アカウントは承認待ちリクエストから手動承認する

global admin はイベント ID を切り替えても管理画面へ入れるため、年度切替ごとに Firebase Console で初期管理者を作り直す必要はありません。
